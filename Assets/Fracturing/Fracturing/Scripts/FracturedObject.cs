...
public class FracturedObject: MonoBehaviour {
  ...
  [HideInInspector]
  public List<UltimateFracturing.SupportPlane> ListSupportPlanes   = new List<UltimateFracturing.SupportPlane>();
  [HideInInspector]
  public List<FracturedChunk>                  ListFracturedChunks = new List<FracturedChunk>();

  [HideInInspector]
  public GameObject SingleMeshObject            = null;  // Optimization. When no chunks have been deattached, we draw the structure using this object, otherwise the chunks individually.
  [HideInInspector]
  ...
  public void ComputeChunksRigidbody() {
    foreach(FracturedChunk chunk in ListFracturedChunks) {
      if(chunk) {
        if(chunk.GetComponent<Rigidbody>()) {
          var RB = chunk.GetComponent<Rigidbody>();
          float fMass = TotalMass * chunk.RelativeVolume;
          if(fMass < 0.001f) fMass = 0.001f;
          float fDrag = TotalDrag * chunk.RelativeVolume;
          if(fDrag < 0.001f) fDrag = 0.001f;
          RB.mass = fMass;
          RB.drag = fDrag;
