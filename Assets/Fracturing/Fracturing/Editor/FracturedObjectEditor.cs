...
public class FracturedObjectEditor: Editor {
  ...
  SerializedProperty PropTotalDrag;
  SerializedProperty PropInterpolation;
  ...
  [MenuItem("GameObject/Fractured Object")]
  ...
  void OnEnable() {
    ...
    PropTotalDrag = serializedObject.FindProperty("TotalDrag");
    PropInterpolation = serializedObject.FindProperty("Interpolation");
  ...
  public override void OnInspectorGUI() {
    ...
    if(fracturedComponent.GUIExpandMain) {
      ...
      PropTotalDrag.floatValue = EditorGUILayout.FloatField(new GUIContent("Total Drag", "The total drag of the object. Each chunk drag will be computed depending on its size and this value."), PropTotalDrag.floatValue);
      PropInterpolation.enumValueIndex = (int)(RigidbodyInterpolation)EditorGUILayout.EnumPopup(new GUIContent("Interpolation", "Rigidbody interpolation type for chunks"), (RigidbodyInterpolation)PropInterpolation.enumValueIndex);
      if(EditorGUI.EndChangeCheck()) {
        fracturedComponent.ComputeChunksRigidbody();
      }
